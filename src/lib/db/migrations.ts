...
export type MigrationOptions = {
  // Borra también las votaciones sobrantes que tengan rankings.
  purgeExtraVotings?: boolean;
  // Cada paso de la migración. Por defecto va a la consola; la ruta de
  // administración lo recoge para devolvérselo al panel.
  log?: (message: string) => void;
};

// Contexto de la ejecución en curso. Las funciones auxiliares lo reciben en
// vez de leer `process.argv`, para que la migración se pueda lanzar también
// desde la app desplegada.
type Ctx = { purgeExtra: boolean; log: (message: string) => void };
...
async function removeExtraVotings(keeperId: string, ctx: Ctx) {
  ...
  for (const extra of extras.rows) {
    ...
    ctx.log(`Votación sobrante eliminada: ${extra.name} (${rankings} rankings).`);
  }
}

async function migrateLegacyRankings(ctx: Ctx) {
  ctx.log("Migrating legacy rankings.voting (voting_type) → UUID FK…");
...
export async function runMigrations(options: MigrationOptions = {}): Promise<string[]> {
  const lines: string[] = [];
  const ctx: Ctx = {
    purgeExtra: options.purgeExtraVotings ?? false,
    log: (message: string) => {
      lines.push(message);
  ...
  if (isLegacy) await migrateLegacyRankings(ctx);
  // Siempre, no solo al crear la tabla: las columnas que se añadieron después
  // (`previous_positions`, `previous_saved_at`) también hay que aplicarlas a
  // las bases de datos que ya tenían `rankings`.
  ...
  if (keeperId) await removeExtraVotings(keeperId, ctx);
